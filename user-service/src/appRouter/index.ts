import express, { type Express, type Request, type Response, type NextFunction, type RequestHandler, Router, Application } from 'express'
...
type ExtractParams<T extends string> = T extends `${infer _Start}:${infer Param}/${infer Rest}` ? [Param, ...ExtractParams<Rest>] : T extends `${infer _Start}:${infer Param}` ? [Param] : []
...
type TResponses = {
  ...
  properties?: Record<string, { type: string; default?: string | number | object | []; nullable?: boolean }> | Record<string, TResponses> | Record<string, { type: string; properties: TResponses }>
...
class BaseRouter {
  ...
  protected createHandler(handler: RouteHandler<any, any, any>, schemas?: { params?: ZodSchema<any>; body?: ZodSchema<any>; query?: ZodSchema<any> }, schemaObject: Record<string, any> = {}) {
