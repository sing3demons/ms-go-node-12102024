...
// EndLog finalizes the log entry.
func (d *DetailLog) End() {
	endTime := time.Now().Format(time.RFC3339)
	d.EndTime = &endTime
	start, _ := time.Parse(time.RFC3339, d.StartTime)
	end, _ := time.Parse(time.RFC3339, endTime)
	duration := fmt.Sprintf("%v ms", end.Sub(start).Milliseconds())
	d.SetResponseTime(duration)

	logJSON, _ := json.Marshal(d)
	logger.Logger.Info(string(logJSON))

	// Reset events after logging
	d.Events = []LogEvent{}
}

// Finish checks if there are events and finalizes logging.
func (d *DetailLog) Finish() {
	if len(d.Events) > 0 {
		d.End()
	}
}
...
// WrapResponseMethods wraps the response methods for logging.
func (d *DetailLog) WrapResponseMethods(res http.ResponseWriter) {
	// Create a response writer wrapper
	wrapper := &ResponseWriterWrapper{ResponseWriter: res}

	// Finish logging when the response is sent
	defer d.Finish()

	// Set the custom response writer
	res = wrapper
}
